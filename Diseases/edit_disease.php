...
if (isset($_GET['id'])) {
    ...
    // Fetch the current disease details
      $sql = "SELECT id, disease_index, disease_name, disease_symptom, disease_cure, disease_severity, crop_id FROM diseases WHERE id = ?";    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("i", $disease_id);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($disease_id, $disease_index, $disease_name, $disease_symptom, $disease_cure, $disease_prevention, $crop_id);
        $stmt->fetch();
        $stmt->close();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Get the form values
    $disease_name = $_POST['disease_index'];
    ...
    $sql = "UPDATE diseases
            SET disease_index = ?, disease_name = ?, disease_symptom = ?, disease_cure = ?, disease_prevention = ?, crop_id = ?
            WHERE id = ?";

    // Prepare statement
    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters
        $stmt->bind_param("ssssii", $disease_index, $disease_name, $disease_symptom, $disease_cure, $disease_prevention, $crop_id, $disease_id);
    ...
    <div class="content">
        ...
        <form action="edit_disease.php?id=<?php echo $disease_id; ?>" method="POST">
          ...
          <div class="form-group">
              <label for="disease_index">Disease Index</label>
              <input type="text" class="form-control" id="disease_index" name="disease_index" value="<?php echo htmlspecialchars($disease_index); ?>" required>
          </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="disease_prevention" name="disease_prevention" rows="4" required><?php echo htmlspecialchars($disease_prevention); ?></textarea>
            </div>
